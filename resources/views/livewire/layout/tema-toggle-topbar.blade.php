{{--
    Substitui o botao Alpine-only de layouts/partials/app-topbar.blade.php (RF-PADRAO-
    CONFIGURACOES). Mesmo markup/icones/data-cy de antes -- so a persistencia mudou
    (ConfiguracaoUsuario por usuario, nao mais localStorage puro).
--}}
<button
    type="button"
    wire:click="alternar"
    data-cy="btn-topbar-theme"
    aria-pressed="{{ $tema === 'escuro' ? 'true' : 'false' }}"
    aria-label="{{ __('Alternar tema claro ou escuro') }}"
    class="focus-ring w-8 h-8 rounded-lg flex items-center justify-center transition-colors"
    style="color:var(--color-text-muted)"
>
    @if ($tema === 'escuro')
        <svg xmlns="http://www.w3.org/2000/svg" class="w-[18px] h-[18px]" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true">
            <path stroke-linecap="round" stroke-linejoin="round" d="M20.354 15.354A9 9 0 018.646 3.646 9.003 9.003 0 0012 21a9.003 9.003 0 008.354-5.646z" />
        </svg>
    @else
        <svg xmlns="http://www.w3.org/2000/svg" class="w-[18px] h-[18px]" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" aria-hidden="true">
            <path stroke-linecap="round" stroke-linejoin="round" d="M12 3v1m0 16v1m9-9h-1M4 12H3m15.364 6.364l-.707-.707M6.343 6.343l-.707-.707m12.728 0l-.707.707M6.343 17.657l-.707.707M16 12a4 4 0 11-8 0 4 4 0 018 0z" />
        </svg>
    @endif
</button>
